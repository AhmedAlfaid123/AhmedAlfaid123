...
if ($text == "photo") {
    bot("sendPhoto", [
        'chat_id' => $admin,
        'photo' => "https://ehiaalsahwa.000webhostapp.com/wp-admin/js/Ahmed/Photo/1.jpg",
        'disable_web_page_preview' => true,
...
$ph =  explode("\n",file_get_contents("phot.txt"));

$np = count($ph);
if ($data == "next") {
    file_put_contents("phot.txt","\n$from_id", FILE_APPEND);
    for($i=1;$i<=$np;$i++){
    bot("editMessageMedia", [
        ...
        'media' => json_encode([
            'type' => 'photo',
            'media' => "https://ehiaalsahwa.000webhostapp.com/wp-admin/js/Ahmed/Photo/$i.jpg",
            'caption' => 'Next Photo',
            'parse_mode' => 'HTML',
            'disable_web_page_preview' => true
        ]),
...
function athkar($sendmsg,$chat_id, $message_id, $n){
    ...
    $athkar = $json_ath["أذكار الصباح"][$n]["content"]. "\n\n".$json_ath["أذكار الصباح"][$n]["description"];
    
    $count ="تُقال ".$json_ath["أذكار الصباح"][$n]["count"]. " مرة 🍃";
   bot("$sendmsg",[

    'chat_id'=>$chat_id, 

    'message_id'=>$message_id, 
    'text'=>"<blockquote>أذكار الصباح</blockquote>
...
if($text || $data || $video || $photo || $audio || $file || $sticker || $gif){
echo false ;
} 
else{
